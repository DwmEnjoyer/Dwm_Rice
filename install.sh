...
dependencies() {
  ...
  then
    sudo pacman -S eza pavucontrol neovim xorg-xrandr python-pip python-pynvim bc git base-devel fontconfig xorgproto libx11 libxft libxinerama ttf-jetbrains-mono ttf-font-awesome feh xorg-xsetroot xorg-setxkbmap network-manager-applet dunst thunar xarchiver thunar-archive-plugin rofi doas cbatticon dash lxappearance sddm qbittorrent papirus-icon-theme nodejs npm ttf-hack-nerd cmake meson libev uthash libconfig stow
    ...
    yay -S otf-takao volctl pnmixer mojave-gtk-theme-git
  else
